...
for suite in catalog explorer vehicle_geometry; do
  ...
  grep -Eq '(CATALOG|EXPLORER|VEHICLE GEOMETRY): [0-9]+ checks, 0 failures' "build/$suite-tests.log"
done
...
for suite in camera_gestures camera_integration crawlworks_visuals; do
  timeout 120 "$godot_bin" --headless --path . --script "tests/$suite.gd" 2>&1 | tee "build/$suite-tests.log"
  if grep -Eq 'SCRIPT ERROR|Parse Error|FAIL:' "build/$suite-tests.log"; then exit 1; fi
  grep -Eq '(CAMERA GESTURES|CAMERA INTEGRATION|CRAWLWORKS VISUALS): [0-9]+ checks, 0 failures' "build/$suite-tests.log"
done
...
timeout 600 xvfb-run -a "$godot_bin" --path . --audio-driver Dummy --rendering-method gl_compatibility --disable-vsync --fixed-fps 30 --write-movie build/driving-review.avi --script tests/driving_video.gd 2>&1 | tee build/driving-review.log
grep -q 'DRIVING REVIEW:' build/driving-review.log
if grep -Eq 'SCRIPT ERROR|Parse Error|ERROR:' build/driving-review.log; then exit 1; fi
ffmpeg -y -i build/driving-review.avi -an -c:v libx264 -preset fast -crf 23 -pix_fmt yuv420p -movflags +faststart build/driving-review.mp4 > build/video-encode.log 2>&1
rm build/driving-review.avi
timeout 600 xvfb-run -a "$godot_bin" --path . --audio-driver Dummy --rendering-method gl_compatibility --disable-vsync --fixed-fps 30 --write-movie build/crawling-review.avi --script tests/crawling_video.gd 2>&1 | tee build/crawling-review.log
grep -q 'CRAWLING REVIEW:' build/crawling-review.log
if grep -Eq 'SCRIPT ERROR|Parse Error|ERROR:' build/crawling-review.log; then exit 1; fi
ffmpeg -y -i build/crawling-review.avi -an -c:v libx264 -preset fast -crf 23 -pix_fmt yuv420p -movflags +faststart build/crawling-review.mp4 > build/crawl-video-encode.log 2>&1
rm build/crawling-review.avi
...
"$godot_bin" --headless --path . --export-debug Android build/bolt-yard-1.0.0-crawlworks.apk 2>&1 | tee build/export.log
test -s build/bolt-yard-1.0.0-crawlworks.apk
python3 - <<'PY'
import zipfile
with zipfile.ZipFile('build/bolt-yard-1.0.0-crawlworks.apk') as archive:
...
PY
"$ANDROID_HOME/build-tools/34.0.0/apksigner" verify --verbose build/bolt-yard-1.0.0-crawlworks.apk | tee build/signature.log
"$ANDROID_HOME/build-tools/34.0.0/aapt" dump badging build/bolt-yard-1.0.0-crawlworks.apk > build/package-info.log
sha256sum build/bolt-yard-1.0.0-crawlworks.apk > build/SHA256SUMS.txt
