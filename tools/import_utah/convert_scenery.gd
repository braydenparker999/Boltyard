extends SceneTree
const OUT = "res://assets/utah/scenery/"
var defs: Dictionary
var mats: Dictionary = {}
var report: Array = []
...
func run():
 ...
 var data=FileAccess.open("res://data/utah/scenery_collision.bin",FileAccess.WRITE)
 data.store_32(0x33545542);data.store_32(models.size())
 ...
 for id in models.size():
  ...
  if collisions.is_empty() and (name.contains("rock") or name.contains("cliff") or name.contains("boulder")):collisions=levels[keys[0]]
 ...
 data.close()
 var meta=FileAccess.open("res://data/utah/scenery_models.json",FileAccess.WRITE)
