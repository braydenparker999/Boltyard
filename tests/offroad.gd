...
func run() -> void:
	...
	check(nodes.size() == 100, "vehicle has 100 physical mass nodes")
	check(core.get_beams().size() > 400, "vehicle contains a structural beam network")
	check(core.get_wheel_hubs().size() == 4, "four independent tire structures")
	...
	check(moving.position.z < origin.z - 1.0, "node-based tire propulsion moves forward")
