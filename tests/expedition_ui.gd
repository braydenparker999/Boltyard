...
func run() -> void:
	...
	check(scene.map_buttons.size() == 3 and scene.map_buttons.has("rockies") and scene.map_buttons.has("russia") and scene.map_buttons.has("redrock"), "the primary destination chooser exposes every region")
	...
	scene.select_map("redrock")
	await settle()
	check(scene.selected_map == "redrock" and scene.world.map_mode == 6 and scene.truck.core.get_terrain_mode() == 6, "choosing the desert changes both scenery and the authoritative native terrain mode")
	check(scene.landmarks.size() >= 6 and str(scene.landmarks[0].id).begins_with("redrock_"), "the desert publishes its own landmarks under its own save keys")
	check(not mountain_id in scene.discovered and not russian_id in scene.discovered, "a third region starts with its own empty progress")
	var desert_id = str(scene.landmarks[3].id)
	scene.discovered.append(desert_id)
	scene.select_destination(desert_id)
	# Six connected routes are the point of this region; the atlas has to draw
	# all of them, not just the wash loop the trailhead sits on.
	var desert_routes: Array = scene.truck.core.get_expedition_routes(6)
	check(desert_routes.size() == 6, "the atlas receives every declared desert route")
	scene.select_map("rockies")
	await settle()
	check(mountain_id in scene.discovered and scene.destination == mountain_id and not russian_id in scene.discovered and not desert_id in scene.discovered, "returning to the mountains restores only its matching progress")
	...
	check(desert_id in scene.exploration_progress.redrock.discovered, "relaunch also retains the inactive desert discoveries")
