...
func run() -> void:
	...
	check(scene.selected_map == "rockies" and scene.world.map_mode == 4 and scene.truck.core.get_terrain_mode() == 4, "an existing garage starts the mountain expedition")
	...
	var expected_maps := 3 + int(FileAccess.file_exists("res://data/gridmap/height.bin")) + int(FileAccess.file_exists("res://data/utah/height.bin"))
	check(scene.map_buttons.size() == expected_maps and scene.map_buttons.has("canyon") and scene.map_buttons.has("rockies") and scene.map_buttons.has("russia"), "the primary destination chooser exposes the three expedition regions")
