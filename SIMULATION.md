...
For solid axles, each shock's rest eye length is measured between affine chassis/carrier mounts. The tower is raised when needed so the configured bump and droop fit a single telescoping damper, eye clearances and piston overlap. Spring rate and separate compression/rebound damping remain applied along this true axis (so wheel rate depends on the motion ratio). Viscous decay uses the attachment effective mass and an exponential step. Compression has digressive high-speed blow-off; rebound uses its independently selected linear coefficient. A progressive bumper dissipates additional energy during compression near the end of stroke; it does not apply extra rebound damping. Free spring force remains compression-only: at droop, the rendered captive spring keeps its free length rather than stretching in tension. Spring preload remains zero; pressure and spring rates remain uncalibrated game settings.
